...
function render(info) {
  // Use youtube-nocookie with autoplay so the video plays immediately
  const embed = (info.embed_url || '').replace('youtube.com/embed/', 'youtube-nocookie.com/embed/') + '&autoplay=1';
...
async function handleDownload(btn) {
  ...
  try {
    // Step 1: request download -> get progressId
    ...
    // Step 2: poll progress until downloadUrl is ready
