...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // Poll progress to get the title + download_url
    const progressData = await pollProgress(data.progress_url, () => {});
    currentTitle = progressData.title || 'فيديو يوتيوب';
    const downloadUrl = progressData.download_url;

    render({ title: currentTitle, downloadUrl });
    loading.hidden = true;
    result.hidden = false;
  } catch (err) {
...
function render(info) {
  // Video plays from external download URL (not YouTube) — under the search
  videoPlayer.src = info.downloadUrl;
...
/* ===== Download flow: request -> redirect to external page immediately ===== */
async function handleDownload(btn) {
  ...
  try {
    ...
    // Step 3: redirect to the external download page — browser starts downloading immediately
