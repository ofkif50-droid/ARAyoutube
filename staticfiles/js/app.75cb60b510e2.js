...
const AUDIO_QUALITIES = [
  { abr: 320, ext: 'm4a', label: '320 kbps' },
  { abr: 256, ext: 'm4a', label: '256 kbps' },
  { abr: 128, ext: 'm4a', label: '128 kbps' },
];
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    audioSourceUrl = data.audio_source_url || '';
    render(data);
    loading.hidden = true;
    result.hidden = false;
  } catch (err) {
...
function renderVideoFormats(videoDownloads) {
    ...
    item.innerHTML = `
      <div class="format-left">
        ...
        <div class="format-detail">
          ...
          <span class="sub">تنزيل خارجي مباشر · ملف مدمج</span>
        </div>
      </div>
  ...
  if (!any) {
    box.innerHTML = '<div class="empty">تعذّر الحصول على روابط تنزيل خارجية لهذه الجودات. حاول مرة تانية.</div>';
  }
}
...
function renderAudioFormats() {
  ...
  box.innerHTML = '';
  if (!audioSourceUrl) {
    box.innerHTML = '<div class="empty">تعذّر الحصول على رابط مصدر للصوت.</div>';
    return;
  }
  AUDIO_QUALITIES.forEach((q) => {
    const item = document.createElement('div');
    item.className = 'format-item';
    const dlUrl = `/download/audio/${currentVideoId}/?height=${q.abr}&source=${encodeURIComponent(audioSourceUrl)}&name=${encodeURIComponent(currentFilename)}`;
    item.innerHTML = `
      <div class="format-left">
        <div class="format-badge audio"><i class="fa-solid fa-music"></i></div>
        <div class="format-detail">
          <span class="res">صوت · ${q.ext.toUpperCase()} ${q.label}</span>
          <span class="sub">استخراج الصوت على السيرفر</span>
        </div>
      </div>
      <div class="format-right">
        <span class="format-size unknown"><i class="fa-solid fa-weight-hanging"></i> ~</span>
