...
const VIDEO_QUALITIES = [
  { format: '4k', label: '4K', note: '4K Ultra HD · WebM + صوت' },
...
const AUDIO_QUALITIES = [
  { format: 'mp3', label: 'MP3', note: 'MP3 · صوت فقط' },
  { format: 'm4a', label: 'M4A', note: 'M4A · صوت فقط' },
];
...
function render(info) {
  // Use standard youtube.com/embed with autoplay — no nocookie/modestbranding (causes Error 153)
...
function renderAudioFormats() {
  ...
  AUDIO_QUALITIES.forEach((q) => {
    ...
    item.innerHTML = `
      ...
      <div class="format-right">
        <button class="fmt-btn download" data-format="${q.format}">
          <i class="fa-solid fa-download"></i> تنزيل
        </button>
      </div>`;
    box.appendChild(item);
