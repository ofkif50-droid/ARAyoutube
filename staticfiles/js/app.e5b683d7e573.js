...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // Video plays in the same page under the search — via our proxy (mode=play)
    // The proxy transcodes AV1/VP9 to H.264 so the browser can play video+audio
...
videoPlayer.addEventListener('canplay', () => { playerLoading.hidden = true; });
videoPlayer.addEventListener('error', () => {
  playerLoading.querySelector('p').textContent = 'تعذّر تشغيل الفيديو — حاول مرة تانية';
});
...
/* ===== Download flow: request -> poll -> open proxy in new tab (mode=download) ===== */
