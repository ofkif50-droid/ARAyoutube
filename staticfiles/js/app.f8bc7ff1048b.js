...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // Video plays in the same page under the search — via our proxy (mode=play -> inline)
...
/* ===== Download flow: request -> poll -> open proxy in new tab (mode=download) =====
   Opening in a new tab keeps the video playing in the same page. */
async function handleDownload(btn) {
  ...
  try {
    ...
    // Open in a new tab so the video keeps playing in the current page
