...
const VIDEO_QUALITIES = [
  ...
  { format: '240', label: '240', note: '240p · MP4 + صوت' },
  { format: '144', label: '144', note: '144p · MP4 + صوت' },
];
  ...
  VIDEO_QUALITIES.forEach((q) => {
    ...
    item.innerHTML = `
      ...
      <div class="format-right">
        <button class="fmt-btn gold" data-url="${encodeURIComponent(`https://www.youtube.com/watch?v=${currentVideoId}`)}" data-format="${q.format}">
...
function renderAudioFormats() {
  ...
  AUDIO_QUALITIES.forEach((q) => {
    ...
    item.innerHTML = `
      ...
      <div class="format-right">
        <button class="fmt-btn download" data-url="${encodeURIComponent(`https://www.youtube.com/watch?v=${currentVideoId}`)}" data-format="${q.format}" data-bitrate="${q.bitrate}">
          <i class="fa-solid fa-download"></i> تنزيل
        </button>
      </div>`;
    box.appendChild(item);
...
async function handleDownload(btn) {
  ...
  try {
    const res = await fetch('/api/download/', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url, format: fmt, audio_quality: bitrate }),
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'فشل التحضير');
    if (data.download_url) {
      window.open(data.download_url, '_blank', 'noopener,noreferrer');
      btn.innerHTML = '<i class="fa-solid fa-check"></i> تم التنزيل';
    } else {
      throw new Error('لم يتم الحصول على رابط التنزيل');
    }
  } catch (err) {
